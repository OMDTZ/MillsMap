...
const createMap = () => {
  ...
  var accessToken =
    "pk.eyJ1IjoiYWxpY28xMSIsImEiOiJja2xhazBneTIwNmFrMm9xcnN2YXMxcHY4In0.43WpCsYZZoQlKlKrBcpqBA";
  ...
  var osmLayer = L.tileLayer(
    "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",
    {
      maxZoom: 18,
      attribution:
        '&copy; <a href="https://openstreetmap.org/copyright" target="_blank">OpenStreetMap contributors' +
        "</a> ",
      tileSize: 256,
    }
  );
  ...
  var mqi = L.tileLayer(
    "http://{s}.mqcdn.com/tiles/1.0.0/sat/{LOD}/{X}/{Y}.png",
    {
...
function drawMarkers(data) {
  ...
  var facilitiesGroup = facilities.group().reduce(
    function (p, v) {
      // add
      p.coordinatesDescription_coodinates_coordinates =
        v.coordinatesDescription_coodinates_coordinates;
      p.school_details_school_name = v.school_details_school_name;
      p.school_details_Location_addr_region =
        v.school_details_Location_addr_region;
      p.school_details_Location_addr_district =
        v.school_details_Location_addr_district;
      p.school_details_Location_addr_ward_shehiya =
        v.school_details_Location_addr_ward_shehiya;
      p.school_details_school_name = v.school_details_school_name;
      p.school_details_Location_addr_subward =
        v.school_details_Location_addr_subward;
      p.geo = v.geo;
      ++p.count;
      return p;
    },
  ...
  mapChart
    ...
    .popup(function (kv) {
      var tooltip =
        "<dt>School name: " +
        kv.value.school_details_school_name +
        "</dt>" +
        "<dt>Region: " +
        kv.value.school_details_Location_addr_region +
        "</dt>" +
        "<dt>District: " +
        kv.value.school_details_Location_addr_district +
        "</dt>" +
        "<dt>Ward: " +
        kv.value.school_details_Location_addr_ward_shehiya +
        "</dt>" +
        "<dt>Subward: " +
        kv.value.school_details_Location_addr_subward +
        "</dt>";
      return tooltip;
    })
    .filterByArea(true)
